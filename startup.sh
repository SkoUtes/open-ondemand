...
ssh-keyscan -t dsa 2> /dev/null ;} >> /etc/ssh/ssh_known_hosts
{ echo -n "ondemand-island,"; \
$(printenv $(echo -n "$INSTANCE_NAME" | tr [:lower:] [:upper:] | tr '-' '_' && echo -n "_ISLAND_SERVICE_HOST")) && \
ssh-keyscan -t ecdsa 2> /dev/null ;} >> /etc/ssh/ssh_known_hosts
{ echo -n "ondemand-island,"; \
$(printenv $(echo -n "$INSTANCE_NAME" | tr [:lower:] [:upper:] | tr '-' '_' && echo -n "_ISLAND_SERVICE_HOST")) && \
ssh-keyscan -t ssh_host_ed25519_key 2> /dev/null ;} >> /etc/ssh/ssh_known_hosts
{ echo -n "ondemand-island,"; \
$(printenv $(echo -n "$INSTANCE_NAME" | tr [:lower:] [:upper:] | tr '-' '_' && echo -n "_ISLAND_SERVICE_HOST")) && \
ssh-keyscan -t rsa 2> /dev/null ;} >> /etc/ssh/ssh_known_hosts
