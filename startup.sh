...
cat /etc/ssh/ssh_host_ecdsa_key.pub ;} >> ssh_known_hosts
{ echo -n "open-ondemand,"; \
printenv -0 $(echo -n "$INSTANCE_NAME" | tr [:lower:] [:upper:] | tr '-' '_' && echo -n "_CLUSTER_IP_SERVICE_HOST") && \
cat /etc/ssh/ssh_host_ed25519_key.pub ;} >> ssh_known_hosts
{ echo -n "open-ondemand,"; \
printenv -0 $(echo -n "$INSTANCE_NAME" | tr [:lower:] [:upper:] | tr '-' '_' && echo -n "_CLUSTER_IP_SERVICE_HOST") && \
cat /etc/ssh/ssh_host_rsa_key.pub ;} >> ssh_known_hosts
...
$(printenv $(echo -n "$INSTANCE_NAME" | tr [:lower:] [:upper:] | tr '-' '_' && echo -n "_ISLAND_SERVICE_HOST")) ;} >> ssh_known_hosts
{ echo -n "ondemand-island,"; \
